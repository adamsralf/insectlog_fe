            ...
            <div class="ml-3 relative">
              <div>
                <button @click="profile_dropdown = !profile_dropdown" type="button" class="max-w-xs bg-white flex items-center text-sm rounded-full focus:outline-none" id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                  ...
                  <img class="h-8 w-8 rounded-full object-cover" src="https://avatars.githubusercontent.com/u/66381853?v=4" alt="">
                </button>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      current_url: this.$router.currentRoute.path
    }
  },
  watch: {
    $route() {
      this.current_url = this.$router.currentRoute.path
    },
  },
  props: {
    content: {
